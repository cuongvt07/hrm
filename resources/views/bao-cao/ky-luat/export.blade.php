...
<html>
<head>
    <title>Báo cáo kỷ luật {{ request('active_tab') == 'phong-ban' ? 'phòng ban' : 'cá nhân' }}</title>
...
<body>
    <table>
        <thead>
            ...
            <tr>
                @if(request('active_tab') == 'phong-ban')
                <!-- Headers cho kỷ luật phòng ban -->
                <th style="font-weight: bold; background-color: #f4f4f4;">STT</th>
                <th style="font-weight: bold; background-color: #f4f4f4;">Đơn vị</th>
                <th style="font-weight: bold; background-color: #f4f4f4;">Ngày kỷ luật</th>
                <th style="font-weight: bold; background-color: #f4f4f4;">Ngày quyết định</th>
                <th style="font-weight: bold; background-color: #f4f4f4;">Đợt kỷ luật</th>
                <th style="font-weight: bold; background-color: #f4f4f4;">Hình thức kỷ luật</th>
                <th style="font-weight: bold; background-color: #f4f4f4;">Giá trị phạt</th>
                <th style="font-weight: bold; background-color: #f4f4f4;">Mô tả</th>
                @else
                <!-- Headers cho kỷ luật cá nhân -->
                <th style="font-weight: bold; background-color: #f4f4f4;">STT</th>
        ...
        <tbody>
            ...
            @if(request('active_tab') == 'phong-ban')
                @foreach($kyLuatPhongBan as $kyLuat)
                    ...
                    <tr>
                        ...
                        <td>{{ $kyLuat->gia_tri ? 'Phạt tiền' : 'Khác' }}</td>
                        <td>{{ $kyLuat->gia_tri !== null ? number_format($kyLuat->gia_tri, 0, ',', '.') . ' VNĐ' : '-' }}</td>
                        <td>{{ $kyLuat->mo_ta }}</td>
                    </tr>
                    @endforeach
                @endforeach
            @else
                @foreach($kyLuatCaNhan as $kyLuat)
                    @foreach($kyLuat->doiTuongApDung as $doiTuong)
                    <tr>
                        <td>{{ $stt++ }}</td>
